...
if (isset($_POST['reg_item'])) {
  $objeto = mysqli_real_escape_string($db, $_POST['objeto']);
  $emprestadoPara = mysqli_real_escape_string($db, $_POST['emprestadoPara']);
  $dataEmprestado = mysqli_real_escape_string($db, $_POST['dataEmprestado']);

  if (empty($objeto)) { array_push($errors, "Coloque o nome do Objeto"); }
  if (empty($emprestadoPara)) { array_push($errors, "Coloque o nome da Pessoa que pegou o item emprestado"); }
  if (empty($dataEmprestado)) { array_push($errors, "Coloque a data"); }

  $query = "INSERT INTO item (objeto, emprestadoPara, dataEmprestado)
  VALUES('$objeto','$emprestadoPara', '$dataEmprestado')";
  mysqli_query($db, $query);
  header('location: telaDosItemRegistrados.php');
